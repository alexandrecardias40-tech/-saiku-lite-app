(function () {
  ...
  async function fetchView(showMessage = true) {
    ...
    try {
      ...
      const response = await fetch(`/api/dashboard?datasetId=${encodeURIComponent(state.datasetId)}`);
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(`/api/dashboard/dataset/${encodeURIComponent(state.datasetId)}`, {
        method: 'DELETE',
      });
      const payload = await response.json();
      if (!response.ok) {
